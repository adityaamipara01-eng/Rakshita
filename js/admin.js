...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function switchTab(tabId) {
    ...
    } else if (tabId === 'tabPendingAccounts') {
      titleEl.textContent = 'Pending Accounts';
      subtitleEl.textContent = 'Approve or reject pending police officer and cyber agent registrations.';
  ...
  function applyRBAC() {
    ...
    // Filter Pending Accounts tab visibility: only visible for Super Admin
    const pendingTabItem = document.querySelector('.admin-nav-item[data-tab="tabPendingAccounts"]');
    if (pendingTabItem) {
      if (currentRole === 'superadmin') {
        pendingTabItem.style.display = 'flex';
      } else {
        pendingTabItem.style.display = 'none';
        if (document.getElementById('tabPendingAccounts').classList.contains('active')) {
          switchTab('tabVerification');
        }
      }
    }
  ...
  function renderPendingAccounts() {
    const tbody = document.getElementById('pendingAccountsTableBody');
    if (!tbody) return;
    tbody.innerHTML = '';
    ...
    if (pendingUsers.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:var(--text-muted); padding: 20px;">No pending account registrations.</td></tr>`;
      return;
    }

    pendingUsers.forEach((u, index) => {
      const tr = document.createElement('tr');
      const userId = `USR-PND-${100 + index}`;
      const dateStr = u.registeredAt ? new Date(u.registeredAt).toLocaleDateString([], {day:'2-digit', month:'short', year:'numeric'}) : 'N/A';

      tr.innerHTML = `
        <td style="font-family:'Share Tech Mono'; font-weight:bold; color:var(--primary-indigo);">${userId}</td>
        <td><strong>${u.name}</strong><br><span style="font-size:0.75rem; color:var(--text-muted);">${u.email}</span></td>
        <td><span class="admin-badge badge-review">${u.role.toUpperCase()}</span></td>
        <td>${dateStr}</td>
        <td><span class="admin-badge badge-pending">Pending Verification</span></td>
        <td style="text-align:right;">
          <button class="admin-btn admin-btn-primary" onclick="approveAccount('${u.email}')">Approve</button>
          <button class="admin-btn admin-btn-danger" onclick="rejectAccount('${u.email}')">Reject</button>
        </td>
      `;
      tbody.appendChild(tr);
    });
  ...
  window.approveAccount = function(email) {
    ...
    if (index !== -1) {
      ...
      const dept = user.role === 'police' ? 'Police Control Room' : 'Cyber Crime Branch';
      const rank = user.role === 'police' ? 'Patrol Officer' : 'Cyber Investigator';
      const newOff = {
      ...
      pushNotification('success', 'Account Approved', `${user.name} (${user.role}) is now approved and active.`);
      renderPendingAccounts();
      renderOfficersGrid();
    }
  };

  window.rejectAccount = function(email) {
    if (confirm(`Are you sure you want to reject and remove registration for ${email}?`)) {
      let users = JSON.parse(localStorage.getItem('rakshika_users') || '[]');
      const index = users.findIndex(u => u.email === email);
      if (index !== -1) {
        ...
        localStorage.setItem('rakshika_users', JSON.stringify(users));
        pushNotification('danger', 'Account Rejected', `Registration for ${userName} has been rejected.`);
        renderPendingAccounts();
      }
    }
  };
