...
(function () {
  ...
  async function _triggerSOS() {
    ...
    if (window.rakshikaSos) {
      ...
      window.rakshikaSos.triggerInstantSOS((lat, lng) => {
        ...
        // Update map
        if (lat && lng && S.dashMap) {
          S.dashMap.updateUserLocation(lat, lng);
          S.lastLat = lat;
          S.lastLng = lng;
        }
      });
  ...
  function _initMap() {
    ...
    const doLocateMe = async (silent = false) => {
      ...
      if (!pos) { 
        if (!silent) dashToast('⚠ Location Error', 'Could not get your location.', 'warning'); 
        return; 
      }
    ...
    document.getElementById('btnFitAllStations')?.addEventListener('click', () => S.dashMap.fitAll());
    
    // Auto-locate silently on load to show nearest station
    setTimeout(() => doLocateMe(true), 1000);
  }
