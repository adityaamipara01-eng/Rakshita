...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const activateSOSUI = (sosData) => {
    ...
    if (!safePointMarker) {
      safePointMarker = L.marker(SAFE_POINT, { icon: safeIcon }).addTo(map);
      safePointMarker.bindPopup("<b>Nearest Safe Zone</b><br>Police Station");
    }

    if (!routeLine) {
      routeLine = L.polyline(pathCoordinates, { color: '#ef4444', weight: 4, dashArray: '5, 5' }).addTo(map);
    } else {
      routeLine.setLatLngs(pathCoordinates);
    }

    map.setView([startLat, startLng], 15);
    updateDistance(startLat, startLng);
  };

  const updateDistance = (lat, lng) => {
    const dist = Math.sqrt(Math.pow(SAFE_POINT[0]-lat, 2) + Math.pow(SAFE_POINT[1]-lng, 2)) * 111; // rough km
    document.getElementById('safePointDist').textContent = `${dist.toFixed(2)} km`;
  ...
  const handleLocationUpdate = (lat, lng) => {
    ...
    if (routeLine) routeLine.setLatLngs(pathCoordinates);
