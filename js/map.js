  ...
  class UserDashboardMap {
    ...
    _plotStations(userLat, userLng) {
      ...
      stations.forEach(st => {
        ...
        // When user location is active, only show the nearest station
        if (userLat !== null && userLng !== null && !isNearest) {
          return;
        }
        ...
        marker.bindPopup(`
          <div style="min-width:180px;line-height:1.6">
            ...
            <div style="margin-top:8px">
              ...
              &nbsp;&nbsp;
              <a href="https://www.google.com/maps/dir/?api=1&destination=${st.lat},${st.lng}" target="_blank" style="color:#10b981;font-size:.75rem;font-weight:600">🗺 Navigate</a>
            </div>
          </div>`, { maxWidth: 240 }
        );

        // Store nearest marker ref — popup opened after flyTo in updateUserLocation
        if (isNearest) this._nearestMarker = marker;
    ...
    updateUserLocation(lat, lng) {
      ...
      // Fly to nearest police station if found, otherwise default to user location
      if (nearest) {
        this._map.flyTo([nearest.lat, nearest.lng], 15, { duration: 1.5 });
      } else {
        this._map.flyTo(latlng, 14, { duration: 1.5 });
      }

      // Open nearest station popup AFTER flyTo animation ends (1.5s + 300ms buffer)
      if (nearest && this._nearestMarker) {
        setTimeout(() => {
          try { this._nearestMarker.openPopup(); } catch(e) {}
        }, 1900);
      }
    ...
    _renderStationList(userLat, userLng) {
      ...
      // If user location is active, show ONLY the single nearest station.
      const limit = (userLat !== null && userLng !== null) ? 1 : ((this._stateFilter || this._searchQuery) ? 30 : 5);
    ...
    clearUserLocation() {
      ...
      if (this._userCircle) {
        this._map.removeLayer(this._userCircle);
        this._userCircle = null;
      }
      if (this._routeLine) {
        this._map.removeLayer(this._routeLine);
      ...
      // Re-plot all stations
      this._plotStations(null, null);
      this._renderStationList(null, null);

      // Reset view to default center
      this._map.setView(this.opts.center, this.opts.zoom);
    }
