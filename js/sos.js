  ...
  startLiveTrackingLoop(activeId) {
    ...
    const performTracking = async () => {
      ...
      if (window.rakshikaDb && window.rakshikaDb.isMock) {
        ...
        // Save coordinates to sessionStorage so they survive page refresh
        ...
        if (window.dashMap) {
          const nearest = window.dashMap.updateUserLocation(lat, lng);
          if (nearest) {
...
// Resume live tracking if an SOS session was active before refresh
if (sessionStorage.getItem('rakshika_active_sos_id')) {
  const activeId = sessionStorage.getItem('rakshika_active_sos_id');
  window.rakshikaSos.state = 'active';
  window.rakshikaSos.activeSessionId = activeId;
  window.rakshikaSos.startLiveTrackingLoop(activeId);
  console.log('SOS Service: Resumed live tracking for session', activeId);
}
